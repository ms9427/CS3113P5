...
{
    if (g_current_scene != g_menu && g_current_scene != NULL) {
        int previous_lives = g_current_scene->m_state.player->num_lives;
        std::cout << previous_lives;
        g_current_scene = scene;
        g_current_scene->initialise(); // DON'T FORGET THIS STEP!
        g_current_scene->update_lives(previous_lives);
    }
    else {
        g_current_scene = scene;
        g_current_scene->initialise(); // DON'T FORGET THIS STEP!
...
void update()
{
    ...
    g_view_matrix = glm::mat4(1.0f);
    g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-g_current_scene->m_state.player->get_position().x, 3.0f, 0.0f));
}
